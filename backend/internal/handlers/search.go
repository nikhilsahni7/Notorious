...
func extractMobileNumber(query string) (string, bool) {
	...
	if colonIdx := strings.IndexRune(query, ':'); colonIdx != -1 {
		field := strings.TrimSpace(query[:colonIdx])
		value := strings.TrimSpace(query[colonIdx+1:])

		// Check if it's a mobile or alt field with a valid mobile number
		if (strings.ToLower(field) == "mobile" || strings.ToLower(field) == "alt") && isMobileNumber(value) {
			return value, true
		}
	}
...
// findChar and toLower replaced by strings.IndexRune and strings.ToLower (stdlib).
...
func (h *SearchHandler) Search(c *gin.Context) {
	...
	if totalResults > 0 && !isDuplicate {
		...
		// Write search history asynchronously — audit record doesn't need to block response.
		// Uses context.Background() because c.Request.Context() is cancelled when response is sent.
		go func() {
			ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
			defer cancel()
			h.searchHistoryRepo.Create(ctx, history)
		}()
...
// Uses stdlib string operations for O(n) performance instead of += concatenation.
